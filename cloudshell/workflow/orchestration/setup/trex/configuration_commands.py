...
RESOURCE_COMMAND_CONFIGURE_MODELS = ["Cisco TRex Chassis"]
...
def execute_autoload_on_trex(sandbox, components):
    """ Execute autoload on deployed Virtual TRex Chassis """

    deployed_apps_names = [app.deployed_app.Name for app in components.values()]

    resource_details_cache = {app_name: sandbox.automation_api.GetResourceDetails(app_name) for app_name in deployed_apps_names}

    for app_name in deployed_apps_names:
        app_resource_details = resource_details_cache[app_name]

        if app_resource_details.ResourceModelName not in RESOURCE_COMMAND_CONFIGURE_MODELS:
            continue
...
def configure_virtual_chassis(sandbox, components):
    """ Execute additional configuration logic on deployed Virtual TRex Chassis """
